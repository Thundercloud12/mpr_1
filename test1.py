...
def psokiller_rastrigin(x, noise_scale=0.8):
    """
    Rastrigin + multiplicative + heavy-tailed noise
    Designed to destroy PSO's pbest/gbest logic
    """
    ...
    # Heavy-tailed noise (Cauchy-like)
    heavy_noise = noise_scale * np.tan(np.pi * (random.random() - 0.5))

    # Fitness-dependent amplification (kills pbest stability)
    amplified_noise = heavy_noise * (1 + 0.1 * abs(base))

    return base + amplified_noise
...
def run_experiment(optimizer_class, name):
    ...
    for trial in range(TRIALS):
        opt = optimizer_class(
            ...
            iters=ITERATIONS,
            bounds=BOUNDS,
            obj_fn=psokiller_rastrigin
        )

        opt.run()
        best_history = opt.history_best
